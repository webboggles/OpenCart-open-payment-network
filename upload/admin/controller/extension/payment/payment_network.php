...
class ControllerExtensionPaymentPaymentNetwork extends Controller
{
    ...
    public function index()
    {
        ...
        $data['entry_merchant_country_code']    = $this->language->get('entry_merchant_country_code');
    ...
    private function validate()
    {
        ...
        if (!(isset($this->request->post[self::$curi . '_merchantid']) &&
            !empty($this->request->post[self::$curi . '_merchantid']))) {
            $this->error['merchantid'] = true;
        }

        if (!(isset($this->request->post[self::$curi . '_merchantsecret']) &&
            !empty($this->request->post[self::$curi . '_merchantsecret']))) {
            $this->error['merchantsecret'] = true;
        }

        if (!(isset($this->request->post[self::$curi . '_gateway_url']) &&
            !empty($this->request->post[self::$curi . '_gateway_url']))) {
            $this->error['gateway_url'] = true;
        }

        if (!(isset($this->request->post[self::$curi . '_merchant_country_code']) &&
            !empty($this->request->post[self::$curi . '_merchant_country_code']))) {
            $this->error['merchant_country_code'] = true;
        }
