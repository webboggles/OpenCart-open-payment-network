...
class ControllerExtensionPaymentPaymentNetwork extends Controller {
    ...
    private function captureOrder() {
        ...
        $params = array(
            ...
            "amount"                => $amount,
            'type'                  => 1,
            "countryCode"           => $this->config->get(self::$curi . '_merchant_country_code'),
...
SQL;
    ...
    protected function createWallet($response) {
        ...
        // If the customerOCSESSID is missing then return from the function
        if (empty($response['customerOCSESSID'])) {
            return;
        }
...
SQL;
            ...
            if (isset($data['threeDSAuthenticated'])) {
                switch ($data['threeDSAuthenticated']) {
                    ...
                    case "E":
                        ...
                        break;
                    case "I":
                        $authenticatedtext = "Information only.";
                        break;
                    case "R":
                        $authenticatedtext = "Reject transaction.";
                        break;
                    default:
     ...
     * Get Gateway URL
     * 
     * This function get the correct gateway URL based on the integration type
     * 
     * @param string $integrationType
     */
    protected function getGatewayURL(string $integrationType)
    {
        ...
        // Switch on the integration type to retrieve the correct URL
        switch ($integrationType) {
                // Hosted form integration names
            case 'Hosted':
            case 'hosted':
            case 'hosted_v2':
            ...
            case 'hosted_v3':
                ...
                break;
            default:
                throw new \Exception('Integration type unknown');
                break;
        }
    }
}
